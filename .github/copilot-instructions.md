...
- **Savings-goal progress is derived** by `utils/balances.ts#goalProgress`: the linked account's balance, or Σ contributions when unlinked. `SavingsGoal.currentAmount` is a migration remnant that nothing reads — do not start reading or writing it. A **total** across goals goes through `totalGoalProgress`, never `Σ goalProgress`: two goals may link to the same account, and that one pile of money must be counted once.
...
- Read balances through `balanceOf`/`computeBalanceMap`, not `computeBalances(state)[id] ?? 0`: for an unknown id of `'constructor'`/`'toString'` the bracket lookup resolves through `Object.prototype` and returns a function. Import data makes those ids reachable. The context exposes only `balanceOf` for this reason; `computeBalances` returns the record form for tests and serialization.
- **Guard every amount the user types with `utils#isValidAmount`** (`Number.isFinite(n) && n > 0`) and report `INVALID_AMOUNT_MESSAGE`. `parseFloat('')` is `NaN` and `NaN <= 0` is **false**, so a hand-rolled `amount <= 0` check writes a NaN record, and one NaN effect makes that account's balance `NaN` with no way back except deleting the record.
